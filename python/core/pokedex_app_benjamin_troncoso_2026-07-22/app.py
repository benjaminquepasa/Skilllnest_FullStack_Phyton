...
# Función cuando no se encuentra un Pokémon
...
def mostrar_por_nombre(nombre):
    encontrados = [p for p in pokemones if p['nombre'].lower() == nombre.lower()]
    if encontrados:
        return render_template('pokemon.html', pokemones=encontrados)
    return pokemon_no_encontrado(f"El Pokémon '{nombre}' no fue encontrado.")

# Ruta para mostrar un Pokémon por número en la Pokédex
@app.route('/pokemon/<int:p_id>')
def mostrar_por_id(p_id):
    encontrados = [p for p in pokemones if p['id'] == p_id]
    if encontrados:
        return render_template('pokemon.html', pokemones=encontrados)
    return pokemon_no_encontrado(f"El Pokémon con ID #{p_id} no fue encontrado.")

# Ruta para mostrar una cantidad específica de Pokémon
@app.route('/pokemon/cantidad/<int:cant>')
def mostrar_cantidad(cant):
    lista_filtrada = pokemones[:cant]
